  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {activeNav === "visual" && (
          <div className="page visual-page"><section className="visual-grid"><div className="panel template-panel"><div className="panel-head"><div><h2>视觉模板</h2><p>统一品牌，适配移动端信息流</p></div></div>{["快讯卡", "行情异动", "链上证据", "事件时间线"].map((name, i) => <button key={name} className={i === 0 ? "template-choice active" : "template-choice"}><span className={`tpl tpl-${i}`}><i/><b/><em/></span><p><strong>{name}</strong><small>{i === 0 ? "突发事件与公告" : i === 1 ? "价格与成交量" : i === 2 ? "交易与地址证据" : "多节点事件回顾"}</small></p><b>›</b></button>)}</div><div className="panel canvas-panel"><div className="canvas-toolbar"><span>1080 × 1350 · PNG</span><div><button>−</button><b>68%</b><button>＋</button></div></div><div className="artboard"><div className="card-brand"><span>W3</span><b>SIGNAL FORGE</b><em>实时情报</em></div><div className="card-status">● 已确认 · 机构资金</div><h2>{selected.title}</h2><p>{selected.summary}</p><div className="card-number"><span>市场即时反应</span><strong className={selected.marketTone}>{selected.market}</strong></div><div className="card-coins">{selected.assets.map(a => <span key={a}>${a}</span>)}</div><div className="card-chart"><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/></div><footer><span>来源：{selected.sources.slice(0,2).map(s=>s.name).join(" · ")}</span><b>14:32 UTC</b></footer></div><div className="qa-bar"><span>✓ 文本安全区</span><span>✓ 来源完整</span><span>✓ 数值一致</span><button onClick={() => showToast("视觉资产已加入发布包")}>生成发布图片</button></div></div><aside className="panel asset-panel"><div className="panel-head"><div><h2>发布素材包</h2><p>最多 4 张图片</p></div></div>{["结论信息卡", "原始证据截图", "市场反应图表", "事件时间线"].map((a,i)=><div className="asset-row" key={a}><span>{i+1}</span><p><strong>{a}</strong><small>{i === 0 ? "已生成" : i === 1 ? "已裁切" : "待生成"}</small></p><b className={i<2?"ready":"pending"}>{i<2?"✓":"○"}</b></div>)}<button className="primary full" onClick={() => {setActiveNav("queue"); showToast("发布包已送入审核队列");}}>送入审核队列 →</button></aside></section></div>
        )}
