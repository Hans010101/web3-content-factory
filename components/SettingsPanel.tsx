...
export function SettingsPanel({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="page settings-page">
    <section className="settings-hero"><div><span>个人工作空间</span><h2>API 与模型配置</h2><p>每位用户使用自己的服务密钥。浏览器只显示配置状态，后台按当前身份隔离并加密保存。</p></div><aside><strong>{payload.user.displayName}</strong><span>{payload.user.local ? "本地临时配置 · 重启后清空" : payload.user.email.endsWith(".invalid") ? "浏览器独立加密空间" : payload.user.email}</span></aside></section>
