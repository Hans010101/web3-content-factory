...
export async function GET(request: Request) {
  ...
  return json({ ok: operational > 0 && adapters.every((item) => item.status !== "unavailable" || item.requiresConfig.length > 0), service: "web3-intel-matrix", checkedAt: new Date().toISOString(), cached: doctor.cached, summary: { total: adapters.length, operational, needsConfig: adapters.filter((item) => item.status === "needs_config").length, degraded: adapters.filter((item) => item.status === "degraded").length, unavailable: adapters.filter((item) => item.status === "unavailable").length, routed: adapters.filter((item) => item.backends.length > 1).length }, adapters }, { headers: { "cache-control": "no-store" } });
}
